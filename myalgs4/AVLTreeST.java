 ...
 *  by John Rogers for DePaul's CSC 301 and CSC 403, DePaul's Data Structures II
 *  classes.
 *
 ...
 *  (counted in number of edges) from this node to each leaf below it.  An
 *  equivalent definition is that the height of a node is one plus the larger
 *  of the heights of its children, where a missing child has height -1.
 *
 ******************************************************************************/
...
public class AVLTreeST<Key extends Comparable<Key>, Value> {
    ...
    /**
     * Returns value associated with the given key in the subtree or null.
     */
    ...
    private Node balance(Node node) {
        if (balanceDifference(node) < -1) {
        	// The new node was inserted to the right; in case B or D
            if (balanceDifference(node.right) > 0) {
            	// The new node was inserted to the left of the right child; in case D
                node.right = rotateRight(node.right);
            } // Otherwise it was inserted to the right of the right child
            node = rotateLeft(node); // in case B
        }
        else if (balanceDifference(node) > 1) {
        	// The new node was inserted to the left; in case A or C
            if (balanceDifference(node.left) < 0) {
            	// The new node was inserted to the right of the left child; in case C
                node.left = rotateLeft(node.left);
            } // Otherwise it was inserted to the left of the left child
            node = rotateRight(node); // in case A
        }
        return node;
    }

    /**
     * Returns the balance difference of the subtree. The balance factor is defined
